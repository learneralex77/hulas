                ...
                @if (isset($service) && $service->translations->isNotEmpty())
                    @foreach ($service->translations as $translation)
                        <div class="service-detail-item border rounded p-3 mb-3">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label class="form-label" for="names_{{ $loop->index }}">Name <span
                                ...
                                <div class="col-md-6">
                                    <label class="form-label" for="icons_{{ $loop->index }}">Icon (FontAwesome
                                        Class)</label>
                                    <input type="text"
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            let detailIndex =
                {{ isset($service) && $service->translations->isNotEmpty() ? count($service->translations) : 1 }};
